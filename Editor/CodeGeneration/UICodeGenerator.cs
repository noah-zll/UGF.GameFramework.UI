...
namespace UGF.GameFramework.UI.Editor
{
    ...
    public static class UICodeGenerator
    {
        ...
        /// 生成组件绑定类
        /// </summary>
        public static string GenerateBindingClass(string className, string namespaceName, List<UIComponentBinding> bindings, UGF.GameFramework.UI.UIBaseType baseType)
        {
            ...
            string baseClassName = baseType == UGF.GameFramework.UI.UIBaseType.MonoBehaviour ? "MonoBehaviour" : "UIFormBase";
            sb.AppendLine($"{indent}public partial class {className} : {baseClassName}");
            ...
            if (baseType == UGF.GameFramework.UI.UIBaseType.UIFormBase)
            {
                // 生成BindComponents方法
                ...
                // 重写BindEvents方法
                sb.AppendLine($"{indent}    /// <summary>");
                sb.AppendLine($"{indent}    /// 绑定事件（自动生成）");
                sb.AppendLine($"{indent}    /// </summary>");
                sb.AppendLine($"{indent}    protected override void BindEvents()");
                sb.AppendLine($"{indent}    {{");
                sb.AppendLine($"{indent}        base.BindEvents();");
            ...
            {
                // MonoBehaviour: Generate Awake
                sb.AppendLine($"{indent}    private void Awake()");
                sb.AppendLine($"{indent}    {{");
                sb.AppendLine($"{indent}        InitializeEvents();");
                sb.AppendLine($"{indent}        OnAwakeCustom();");
                sb.AppendLine($"{indent}    }}");
                sb.AppendLine();
            }
            ...
            foreach (var binding in bindings)
            {
                if (binding.IsValid() && binding.BoundEvents != null && binding.BoundEvents.Count > 0)
                {
                    string methodNameBase = char.ToUpper(binding.ComponentName[0]) + binding.ComponentName.Substring(1);
                    sb.AppendLine($"{indent}        // 绑定事件: {binding.ComponentName}");

                    foreach (var evt in binding.BoundEvents)
                    {
                        string suffix = evt.Substring(2); // onClick -> Click, onValueChanged -> ValueChanged
                        string handlerName = $"On{methodNameBase}{suffix}";
                        sb.AppendLine($"{indent}        {binding.ComponentName}.{evt}.AddListener({handlerName});");
                    }
                }
            }
            ...
            {
                sb.AppendLine($"{indent}    partial void OnAwakeCustom();");
            }
        ...
        /// 生成业务逻辑类
        /// </summary>
        public static string GenerateLogicClass(string className, string namespaceName, List<UIComponentBinding> bindings, UGF.GameFramework.UI.UIBaseType baseType)
        {
            var sb = new StringBuilder();
            ...
            foreach (var binding in bindings)
            {
                if (binding.IsValid() && binding.BoundEvents != null && binding.BoundEvents.Count > 0)
                {
                    ...
                    foreach (var evt in binding.BoundEvents)
                    {
                        string suffix = evt.Substring(2);
                        string handlerName = $"On{methodNameBase}{suffix}";
                        string paramsDef = GetEventParameters(binding.ComponentType, evt);

                        sb.AppendLine($"{indent}    /// <summary>");
                        sb.AppendLine($"{indent}    /// {binding.ComponentName} {evt} 事件");
                        sb.AppendLine($"{indent}    /// </summary>");
                        sb.AppendLine($"{indent}    private void {handlerName}({paramsDef})");
                        sb.AppendLine($"{indent}    {{");
                        sb.AppendLine($"{indent}        // TODO: 实现 {binding.ComponentName} {evt} 逻辑");
                        sb.AppendLine($"{indent}    }}");
                        sb.AppendLine();
                    }
                }
        ...
        public static void GenerateCode(UIDesigner designer)
        {
            ...
            // 生成绑定类
            string bindingCode = GenerateBindingClass(designer.UIFormName, designer.NamespaceName, designer.ComponentBindings, designer.BaseType);
            ...
            {
                // 文件不存在时创建新文件
                string logicCode = GenerateLogicClass(designer.UIFormName, designer.NamespaceName, designer.ComponentBindings, designer.BaseType);
        ...
        private static void UpdateLogicClassEvents(string filePath, List<UIComponentBinding> bindings)
        {
            ...
            // 添加缺失的事件处理方法
            content = AddMissingEventHandlers(content, bindings);
        ...
        /// 添加缺失的事件处理方法
        /// </summary>
        private static string AddMissingEventHandlers(string content, List<UIComponentBinding> bindings)
        {
            ...
            foreach (var binding in bindings)
            {
                if (binding.IsValid() && binding.BoundEvents != null && binding.BoundEvents.Count > 0)
                {
                    ...
                    {
                        string suffix = evt.Substring(2);
                        string methodName = $"On{methodNameBase}{suffix}";
                        ...
                        if (!content.Contains(methodName))
                        {
                            ...
                            {
                                string paramsDef = GetEventParameters(binding.ComponentType, evt);
                                string newMethod = $"\n    /// <summary>\n    /// {binding.ComponentName} {evt} 事件\n    /// </summary>\n    private void {methodName}({paramsDef})\n    {{\n        // TODO: 实现 {binding.ComponentName} {evt} 逻辑\n    }}\n";
                                sb.Insert(insertPos, newMethod);
                                content = sb.ToString(); // Update content to search in updated string if needed (though Contains works on original, careful with offsets if multiple insertions)
                                // Actually, better to re-calculate insertPos or just assume it's always valid to insert before last brace.
                                // However, sb.Insert changes offsets.
                                // Simplest way: use sb for contains check? No, sb doesn't have Contains.
                                // Re-find insertPos is safer.
                            }
                        }
                    }
                }
        ...
        /// 获取事件参数定义
        /// </summary>
        private static string GetEventParameters(string componentType, string eventName)
        {
            if (eventName == "onValueChanged")
            {
                if (componentType == "Toggle") return "bool isOn";
                if (componentType == "Slider" || componentType == "Scrollbar") return "float value";
                if (componentType == "Dropdown" || componentType == "TMP_Dropdown") return "int index";
                if (componentType.Contains("InputField")) return "string value";
                if (componentType == "ScrollRect") return "Vector2 position";
            }
